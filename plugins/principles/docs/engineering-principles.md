...
Durable principles and operating rules — the canon the `principles:instill` skill installs into a repo's (or your user profile's) always-on Claude Code context.
...
- **The file keeps the principles active.** [`instill`](../skills/instill/SKILL.md) writes one profile payload to that profile's target: `.claude/rules/engineering-kit.md` for engineering, `.claude/rules/working-with-claude.md` for universal — at repo scope, or under `~/.claude/rules/` to cover every repo you work in. That file auto-loads thereafter. A repo with no `.claude/` directory can take a managed section in the root `CLAUDE.md` instead.
