...
The canon ships as **profiles**: self-contained payload files that the [`instill`](../skills/instill/SKILL.md) skill copies verbatim into a managed block. This file is the contract between the payloads and the installer — the registry, the marker format, the migration rule, and the map that keeps two wordings of one idea from drifting apart.
...
```markdown
<!-- BEGIN canon:<profile> v<VERSION> — managed block. Re-sync with the instill skill; edit above or below, not inside. Source: https://github.com/cacack/claude-code-plugins (principles plugin). -->
...
```markdown
<!-- BEGIN canon:engineering vX.Y.Z — managed block. Re-sync with the instill skill; edit above or below, not inside. Source: https://github.com/cacack/claude-code-plugins (principles plugin). -->
```
