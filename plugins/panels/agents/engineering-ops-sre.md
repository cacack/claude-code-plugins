...
description: Senior SRE/operations reviewer assessing the *whole repository* for observability, deployability, runbook adequacy, on-call burden, failure-mode documentation, and CI/CD health. Intended for use within panels:panel-engineering where 5 personas run in parallel; the orchestrator passes a snapshot.md path and an output file path.
