...
- **Write batch metadata** to `.prompts/.batch.json` for resume capability:
  ...
  **Simple format** (all prompts share one strategy):
  ```json
  {
    ...
    "source": "/create-prompt [original arguments]",
    "prompts": ["001-first-task.md", "002-second-task.md", "003-third-task.md"],
    "completed": []
  }
  ```

  **Execution groups format** (mixed strategies - use when prompts have varying dependencies):
  ```json
  {
    "created": "[ISO timestamp]",
    "source": "/create-prompt [original arguments]",
  ...
  **Format selection:**
  - Use **simple format** when all prompts share the same execution strategy
  - Use **execution groups** when prompts have mixed dependencies (e.g., research in parallel → plan sequentially → implement in parallel)
  - Each execution group (layer) completes before the next layer starts
  - `completed`: Empty array initially, updated by `/run-prompt` as prompts complete
...
<action>
If user chooses #1, invoke via SlashCommand tool: `/delivery:run-prompt 005`
</action>
</single_prompt_scenario>
...
<actions>
If user chooses #1, invoke via SlashCommand tool: `/delivery:run-prompt 005 006 007 --parallel`
If user chooses #2, invoke via SlashCommand tool: `/delivery:run-prompt 005 006 007 --sequential`
</actions>
</parallel_scenario>
...
<actions>
If user chooses #1, invoke via SlashCommand tool: `/delivery:run-prompt 005 006 007 --sequential`
If user chooses #2, invoke via SlashCommand tool: `/delivery:run-prompt 005`
</actions>
</sequential_scenario>
...
- **For multiple prompts**: `.prompts/.batch.json` created with strategy and prompt list
...
- **For multiple prompts**: Always write `.prompts/.batch.json` with execution strategy before presenting decision tree
- After saving, present the decision tree as inline text (not AskUserQuestion)
- Use the SlashCommand tool to invoke /delivery:run-prompt when user makes their choice
</meta_instructions>
