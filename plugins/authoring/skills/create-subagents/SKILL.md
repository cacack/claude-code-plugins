...
The restrictions prevent plugins from escalating privileges or introducing untrusted server connections. Project-level and user-level agents can use every field without restriction. Since this repo ships agents *as plugins*, do not rely on these three fields in `plugins/*/agents/` — they won't take effect.
</plugin_restrictions>
