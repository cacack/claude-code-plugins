...
<quick_start>
```
/cacack:pressure-test                              # I'll ask you to paste or point me at the idea
/cacack:pressure-test docs/strategy/foo.md         # pressure-test a file (plan, CONSTITUTION.md, one-pager)
/cacack:pressure-test We should consolidate the three services into one
```
...
2. **Dispatch to the agent.** Launch the `cacack:rude-qa` subagent via the Task tool with a prompt containing:
