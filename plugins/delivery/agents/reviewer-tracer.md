...
description: Cross-file data-flow reviewer that traces every changed value, field, column, key, or contract to all of its writers and readers repo-wide, then checks whether they still agree. Covers the provenance/reachability gap the diff-local reviewers structurally cannot see. Intended for use within delivery:panel-review where 6 reviewers run in parallel; the orchestrator passes a diff file path.
