...
---
<!-- No allowed-tools restriction: /do is an orchestration command. Batch/prompts mode delegates to /delivery:run-prompt (which uses Task internally to spawn subagents). Direct mode uses TodoWrite + the full file-edit surface (Read/Edit/Write/Glob/Grep/Bash). Tightening this would require auditing both code paths together. -->
...
run `/delivery:play NN` first to plan, then `/delivery:do` to execute.
```
...
- **Subagent dispatch (default)** — invoke `/delivery:run-prompt` via the Skill tool. Each prompt runs in fresh context via `Agent`/Task. Best for: large batches, parallel groups, when current context is already loaded.
...
```
Work complete. Run `/delivery:ship` to commit and create a PR?
```
...
- **all modes**: explicit offer to run `/delivery:ship`
</output>
