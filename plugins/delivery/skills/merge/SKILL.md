...
**One bypass only:** when the *user themselves typed an explicit PR/MR number in this turn's invocation* (e.g. `/delivery:merge 18`), that is the confirmation — skip the prompt. Do **not** treat a number passed by another skill (e.g. `deliver-milestone` delegating `/delivery:merge <n>`) as a user confirmation; in the checkpointed route the human gate lives in `deliver-milestone`'s own pause, and the autonomous route is non-interactive by design (stated up front when that route is chosen). When in doubt, ask.
</step_3_pregate>
...
# Typical cycle tail
/delivery:ship      # opens the PR
# … review / CI …
/delivery:merge     # rebases, merge-commits, tears down the worktree, prunes
```
</examples>
