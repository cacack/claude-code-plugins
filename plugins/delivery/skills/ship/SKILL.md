...
allowed-tools:
  ...
  - AskUserQuestion
  - TodoWrite
  # Skill is granted solely to invoke /delivery:panel-review from the review
...
2. **Freshness check** — refuse to push a stale branch (the `/delivery:merge` flow rebases onto the default before merging; catching staleness now avoids surprise conflicts later):
   ```bash
   git fetch origin
...
**Ensure the branch is current with the default branch before doing any further work.** The `/delivery:merge` flow rebases onto the default and re-gates CI before merging; catching staleness here avoids wasted preflight runs against stale code and lets the user resolve conflicts early.
...
Apply this signal table (kept in sync with `/play`'s Documentation Impact). The concrete doc destinations depend on the project's layout; in this repo they map to `README.md`, `plugins/*/docs/`, and `CHANGELOG.md` / `FEATURES.md` / `IDEAS.md` where present:
...
| Design/architecture/principle decision | project `docs/` (e.g. `plugins/*/docs/`) |
